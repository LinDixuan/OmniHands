...
from .relighted_dataset import Relighted_Hand_Video_V3
...
class Mix_temoral(pl.LightningDataModule):
    def __init__(self, cfg: CfgNode, dataset_cfg: CfgNode) -> None:
        """
        Initialize LightningDataModule for HAMER training
        Args:
        ...
        inter_img_path = "/workspace/twohand_full"
        inter_label_path = "/workspace/twohand_crop"
        #dex_train_dataset = DexYCB_Temporal_Twohand(dataset_root=dex_data_root, train_label_root=dex_label_root,
        #                                    mode="val", T=cfg.MODEL.SEQ_LEN, gap=cfg.MODEL.SEQ_GAP)
        rel_train_dataset = Relighted_Hand_Video_V3(task='train',seg_len=cfg.MODEL.SEQ_LEN,gap=cfg.MODEL.SEQ_GAP)
        inter_train_dataset = get_temp_dataset(img_path=inter_img_path, label_path=inter_label_path, split="train",
                                               hand_type="interacting", T=cfg.MODEL.SEQ_LEN,gap=cfg.MODEL.SEQ_GAP,
                                               fps='30fps')
        inter_val_dataset = get_temp_dataset(img_path=inter_img_path, label_path=inter_label_path, split="test",
                                       hand_type="interacting", T=cfg.MODEL.SEQ_LEN, gap=cfg.MODEL.SEQ_GAP,
                                       fps='30fps')
        h2o3d = H2O3D(split='train', seq_len=cfg.MODEL.SEQ_LEN, gap=15)
        hmr_train_dataset = hamer_filtered_dataset(cfg, dataset_cfg, T=cfg.MODEL.SEQ_LEN)


        self.train_dataset = mix_dataset(dataset_list=[inter_train_dataset, rel_train_dataset, hmr_train_dataset],
                                         dataset_prob=[0.35, 0.35, 0.3])

        self.val_dataset = mix_dataset(dataset_list=[inter_val_dataset], dataset_prob=[1.0])

        self.test_dataset = get_temp_dataset(img_path=inter_img_path, label_path=inter_label_path, split="val",
                                             hand_type="interacting", T=cfg.MODEL.SEQ_LEN,gap=cfg.MODEL.SEQ_GAP)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        Returns:
            torch.utils.data.DataLoader: Validation dataloader
        """
        val_dataloader = torch.utils.data.DataLoader(self.val_dataset, 4, drop_last=True, num_workers=0, shuffle=True)
        return val_dataloader
