...
class Arctic_Multiview(torch.utils.data.Dataset):
    def __init__(self, T=5, split='train'):
        # mano layer
        ...
        self.seq_len = T
        assert self.seq_len <= 8, f"too many views:{self.seq_len}"
        self.split = split
        self.filter_offset = 20
    ...
    def __getitem__(self, index):
        ...
        for hand_type in ['right', 'left']:
            ...
            output = self.mano_layer[hand_type](global_orient=root_pose[hand_type].clone(),
                                                hand_pose=hand_pose[hand_type].clone(), betas=shapes[hand_type].clone(),
                                                transl=trans)
