...
class InterHandSingleFrameDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for capture in capture_list:
            ...
            for seq_name in seq_list:
                ...
                if fps == '30fps' and data['frame_num'] < 15:
                    continue
    ...
    def getitem(self, idx, data):
        ...
        valid_camId_list = data['valid_cameraId_list']
        valid_frameId_list = data['valid_frameId_list']

        frameIdx = valid_frameId_list[idx]
        camIdx = valid_camId_list[idx]
        ...
        if self.split == "train" and self.aug:
            (center_rand, scale_rand, rot_rand, blur_rand, color_jitter_funcs) = \
                (data['center_rand'], data['scale_rand'],
                 data['rot_rand'], data['blur_rand'],
                 data['color_jitter_funcs'])
            ...
            (
                ...
                img,
                mano_param_right.numpy(),
                mano_param_left.numpy(),
                joints2d_right,
                joints2d_left,
                joints3d_right,
                joints3d_left,
            ...
            joints2d_glb_right = joints2d_glb_right / 256 - 0.5
            joints2d_glb_left = joints2d_glb_left / 256 - 0.5
        ...
        #sample['joints2d_glb_right'] = joints2d_glb_right
        #sample['joints2d_glb_left'] = joints2d_glb_left
    ...
    def data_aug(
        ...
        return (img, img_right, img_left, mano_param_right, mano_param_left,
                ...
                joints2d_glb_right, joints2d_glb_left,
    ...
    def __getitem__(self, i):
        ...
        if self.fps == '5fps':
            select_ids = [idx] * self.T
        else:
            select_ids = []
            inverse = bool(random.random() > 0.5) * (self.split == 'train')
            while len(select_ids) < self.T:
                select_ids.append(idx)
                if not inverse:
                    if (idx + self.gap) >= valid_camId_list.shape[0] or valid_camId_list[idx + self.gap] != camIdx:
                        inverse = True
                    else:
                        idx = idx + self.gap
                else:
                    if (idx - self.gap) < 0 or valid_camId_list[idx - self.gap] != camIdx:
                        inverse = False
                    else:
                        idx = idx - self.gap
        camera_name = data['camera_name_list'][camIdx]
        ...
        for i, index in enumerate(select_ids):
            for handtype in ['left', 'right']:
                output = self.getitem(index, data)
        ...
        sample['dataset'] = torch.ones(self.T) * 3
...
def get_temp_dataset(img_path: str, label_path: str, split: str,
                             capture_list: Optional[List[int]] = None,
                             need_image: bool = True,
                             ...
                             T=15,
                             gap=2,
                             fps='30fps') -> InterHandSingleFrameDataset:
