...
class ManoLoss:
    def __init__(
        ...
        self.lambda_consistence = lambda_consistence
    ...
    def compute_loss(self, preds, gts):
        ...
        for i, pred in enumerate(preds):
            for hand_side in ['right', 'left']:
                ...
                if self.lambda_joints2d is not None and f"joints2d_glb_{hand_side}" in gts:
            ...
            if self.lambda_consistence is not None:
                assert pred["verts3d_world_right"].dim() == 3
                bs = pred["verts3d_world_right"].shape[0] // self.seq_len
                v3d_right_pred, v3d_left_pred = pred["verts3d_world_right"], pred["verts3d_world_left"]
                j3d_right_pred, j3d_left_pred = pred["joints3d_world_right"], pred["joints3d_world_left"]
                pose_right_pred = pred["mano_pose_right"].clone()
                pose_left_pred = pred["mano_pose_left"].clone()
                right_aa = pose_right_pred[:, :3]
                left_aa = pose_left_pred[:, :3]
                left_aa = left_aa * torch.FloatTensor([1, -1, -1]).unsqueeze(0).to(left_aa)
                ...
                v3d_right_regul_lr = v3d_right_pred - j3d_left_pred[:, 9:10]
                v3d_right_regul_lr = v3d_right_regul_lr @ rmat_inv_l

                v3d_left_regul_rr = v3d_left_pred - j3d_right_pred[:, 9:10]
                v3d_left_regul_rr = v3d_left_regul_rr @ rmat_inv_r

                v3d_right_regul_lr = v3d_right_regul_lr.reshape(bs, self.seq_len, -1)
                v3d_left_regul_rr = v3d_left_regul_rr.reshape(bs, self.seq_len, -1)
                dismap_r = v3d_right_regul_lr.unsqueeze(2) - v3d_right_regul_lr.unsqueeze(1)
                dismap_l = v3d_left_regul_rr.unsqueeze(2) - v3d_left_regul_rr.unsqueeze(1)

                consist_loss = self.lambda_consistence * \
                               ((dismap_r ** 2).mean() +
                                (dismap_l ** 2).mean())
                
                final_loss += consist_loss
                if i == num_preds - 1:
                    mano_losses[f"consistence_loss"] = consist_loss.detach()


            if self.lambda_rel_joints is not None:
                assert pred["verts3d_world_right"].dim() == 3
                j3d_right_pred, j3d_left_pred = pred["joints3d_world_right"], pred["joints3d_world_left"]
            ...
            if self.lambda_close is not None:
                assert pred["verts3d_world_right"].dim() == 3

                v3d_right_pred, v3d_left_pred = pred["verts3d_world_right"], pred["verts3d_world_left"]
                v3d_right_gt, v3d_left_gt = gts["verts3d_world_right"], gts["verts3d_world_left"]
