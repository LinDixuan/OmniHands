...
from ..components.pose_transformer import TransformerDecoder, Transformer
...
class DynamicFusionModule(nn.Module):
    def __init__(
        ...
        for layer_idx, (inp_neurons, out_neurons) in enumerate(
            ...
            glb_layers.append(nn.Linear(inp_neurons, out_neurons))
            glb_layers.append(nn.LeakyReLU(inplace=True))
            loc_layers.append(nn.Linear(inp_neurons, out_neurons))
            loc_layers.append(nn.LeakyReLU(inplace=True))
        self.mano_glb_layer = nn.Sequential(*glb_layers)
        self.mano_loc_layer = nn.Sequential(*loc_layers)
        # Pose layers
        self.pose_reg = nn.Linear(mano_base_neurons[-1], self.pose6d_size)
        self.root_reg = nn.Linear(mano_base_neurons[-1], self.root6d_size)
        # Shape layers
    ...
    def forward(
        ...
        seq_len = loc_feat.shape[1]
        glb_feat = self.mano_glb_layer(glb_feat).unsqueeze(1)
        loc_feat = self.mano_loc_layer(loc_feat)
        pred_mano_shape = self.shape_reg(glb_feat)      # (S x B) x 1 x 10
        pred_hand_pose_6d = self.pose_reg(glb_feat)     # (S x B) x 1 x C
        pred_root_6d = self.root_reg(loc_feat)          # (S x B) x T x C
        pred_cam = self.cam_reg(loc_feat)               # (S x B) x T x C
        loc_feat = einops.rearrange(loc_feat, '(s b) t c -> b t (s c)', s=2)
        pred_rel = self.rel_reg(loc_feat)   # B x T x 3

        pred_mano_shape = pred_mano_shape.repeat(1, seq_len, 1).flatten(0, 1)
        pred_hand_pose_6d = pred_hand_pose_6d.repeat(1, seq_len, 1).flatten(0, 1)
        pred_root_6d = pred_root_6d.flatten(0, 1)
        pred_cam = pred_cam.flatten(0, 1)   # (S x B x T) x C
        pred_rel = pred_rel.flatten(0, 1)   # (B x T) x C
        pred_mano_pose_6d = torch.cat([pred_root_6d, pred_hand_pose_6d], dim=-1)
        ...
        for k in pred_mano_results.keys():
            ...
            bs = pred.shape[0]
            pred = pred.reshape([2, bs//2, *pred.shape[1:]])
            pred_right = pred[0]
            pred_left = pred[1]
...
class AggregateHead(nn.Module):
    def __init__(self,
        ...
        super(AggregateHead, self).__init__()
        ...
        self.to_token_embedding = nn.Linear(1, channels)

        self.persp_encoder = Transformer(
            dim=channels,
            depth=6,
            heads=16,
            mlp_dim=channels,
    ...
    def forward(self, hand_token, batch_seq, train):
        ...
        for k in batch_seq.keys():
            batch[k] = batch_seq[k].flatten(0, 1)

        hand_token = einops.rearrange(hand_token, '(b t s) c -> (s b) t c', s=2, t=self.seq_len)
        token_glb = torch.ones_like(hand_token[:, 0:1, 0:1])
        token_glb = self.to_token_embedding(token_glb)
        token_input = torch.cat([token_glb, hand_token], dim=1)
        token_out = self.persp_encoder(token_input)  # (S B) (T+1) C
        glb_feat = token_out[:, 0]
        loc_feat = token_out[:, 1:]
        pred_mano_result = self.mano_branch.forward(glb_feat, loc_feat)
